...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in legalActions:
            ...
            if q_val == max_Qval and best_action != None:
                best_action = random.choice([action, best_action])
            elif q_val > max_Qval or best_action == None:
                max_Qval = q_val
                best_action = action
        return best_action
    ...
    def update(self, state, action, nextState, reward):
        ...
        if len(nextLegalActions) == 0:
            ...
            sample = reward + self.discount * max([self.getQValue(nextState, nextAction) for nextAction in nextLegalActions])
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        if len(nextLegalActions) == 0:
            ...
            difference = reward + self.discount * max([self.getQValue(nextState, nextAction) for nextAction in nextLegalActions]) - self.getQValue(state, action)
        for f in featureVector:
